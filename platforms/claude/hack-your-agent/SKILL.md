...
2. Create or refresh `redteam/` with `python3 ../../../scripts/init_redteam_run.py --target "$ARGUMENTS"`.
...
5. Choose only the attack families that are present in the target using `../../../core/references/attack-taxonomy.md` and `../../../core/references/probe-library.md`, then seed a concrete trial matrix with `python3 ../../../scripts/init_redteam_run.py --target "$ARGUMENTS" --families "<family-1>,<family-2>" --force`.
6. Execute the matrix row by row using `../../../core/references/forensic-execution.md`. Every selected family must have at least one paired control and attack run. ultrathink before deciding the family list.
